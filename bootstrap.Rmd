...
zeroPercentC <- vector(mode = "numeric", length=8)
eightyNinePercentC <- vector(mode = "numeric", length=8)
```
...
{
replicates <- sample(c(1:8), replace = TRUE, 8) #sample numbers 1-4 four times with replacement. This vector will be used to randomly pick an expression value from the vector containing all our of experimental values
...
if (Strain == "CRD34") {
  sum(medianEstimate < median(rpos26Vect))/8
} else {
  sum(medianEstimate > median(rpos26Vect))/8
}
